...
def get_keywords_yake(text):
    ...
    unique_keywords = remove_fuzzy_duplicates(
        [keyword[0] for keyword in keywords]
    )
    result = [
        (keyword[0], keyword[1])
        for keyword in keywords
        if keyword[0] in unique_keywords
    ]
    # normalize scores
    ...
    scores = 1 - scores
    return sorted(
        [(keyword[0], score) for keyword, score in zip(result, scores)],
        key=lambda x: x[1],
        reverse=True,
    )
...
def get_keywords(text, n=10):
    ...
    textrank_keywords = [
        (keyword[0], keyword[1]) for keyword in textrank_keywords
    ]
    # combine keywords with summing scores if keyword is in both lists
    # and remove fuzzy duplicates

    unique_keywords = {}
    for keyword in textrank_keywords:
        unique_keywords[keyword[0]] = keyword[1] * 0.65
    for keyword in yake_keywords:
        duplicate_found = False
        for unique_keyword in textrank_keywords:
            if fuzz.ratio(keyword[0], unique_keyword) >= 50:
                unique_keywords[unique_keyword] += keyword[1] * 0.35
                duplicate_found = True
                break
        if not duplicate_found:
            unique_keywords[keyword[0]] = keyword[1] * 0.35
    unique_keywords = sorted(
        unique_keywords.items(), key=lambda x: x[1], reverse=True
    )
    return unique_keywords[:n] + [
        (keyword, score)
        for keyword, score in unique_keywords[n:]
        if score > 0.45
    ]
