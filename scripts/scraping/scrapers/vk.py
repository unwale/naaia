...
class VkParser(Parser):
    ...
    def __init__(self, source: NewsSource):
        ...
        req = Request(
            self.base_url.format(method="wall.get", params=urlencode(params))
        )
    ...
    async def fetch_news(self, item_limit: int = None) -> List[NewsItem]:
        ...
        async with aiohttp.ClientSession() as session:
            for i in range((item_limit // 100) + 1):
                ...
                request_url = self.base_url.format(
                    method="wall.get", params=urlencode(params)
                )
                response = await session.get(request_url)
                response_json: dict = json.loads(await response.text())[
                    "response"
                ]
                if not ("items" in response_json.keys()):
                    continue
                for post in response_json["items"]:
                    ...
                    news.append(
                        NewsItem(
                            ...
                            text=post["text"],
                            source=self.source,
                            news_url=f"https://vk.com/{self.source.id}"
                            + f"?w=wall{post['owner_id']}_{post['id']}",
                        )
                    )
        return news
