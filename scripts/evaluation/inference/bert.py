...
# TODO replace model loading with weights loading
...
class BertTextClassifier():
    ...
    def predict(self, inputs: List[str], topics: List[str], batch_size=8) -> List[str]:
        predictions = []
        ...
        for i in tqdm(range(0, len(inputs), batch_size), desc="Predicting topics", total=len(inputs)//batch_size):
            batch = {key: value[i:i+batch_size] for key, value in tokenized_inputs.items()}
