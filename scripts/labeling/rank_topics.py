...
parser = argparse.ArgumentParser(
    description="Rank topics using OpenAI Batch API"
)
parser.add_argument(
...
parser.add_argument(
    "--path",
    type=str,
    help="Path to jsonl file to rank",
    default="./data/preprocessed/test.jsonl",
)
parser.add_argument(
    "--column",
    type=str,
    help="Name of the column to rank topics by",
    default="text",
)
...
prompt = (
    "Оцени от 1 до 100, насколько каждая из предложенных тем "
    + "релевантна для этого текста (у более релевантной темы должна"
    + ' быть более высокая оценка): "{text}". Темы, которые нужно оценить'
    + " по релевантности: "
    + topics_list
    + ". Отвечай в формате JSON списка из объектов с полями topic и score."
)
...
for i, batch in enumerate(batches):
    prompts = batch[args.column].apply(
        lambda x: prompt.format(text=" ".join(x.split()))
    )
...
data.loc[:, "topic_ranking"] = data.loc[:, "topic_ranking"].apply(
    lambda x: x["topics"]
)
data.loc[:, "topic_ranking"] = data.loc[:, "topic_ranking"].apply(
    lambda x: {item["topic"]: item["score"] for item in x}
)
