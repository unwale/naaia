...
argparser = argparse.ArgumentParser(
    description="Label topics using OpenAI Batch API"
)
    ...
    "schema": {
        ...
        "properties": {
            "temporal_intents": {
                ...
                "items": {
                    "type": "object",
                    "properties": {"temporal_intent": {"type": "string"}},
                },
            }
        },
    },
...
for i, batch in enumerate(batches):
    ...
    for queries in batch["queries"].values:
        prompt = (
            "Классифицируй каждый пользовательский запрос по"
            + f' темпоральному интенту: {", ".join(intents)}.\n\n'
        )
    ...
    temporal_intents = [
        json.loads(label)["temporal_intents"] for label in temporal_intents
    ]
    temporal_intents = [
        [intent["temporal_intent"] for intent in intents]
        for intents in temporal_intents
    ]
    data.loc[batch.index, "temporal_intent"] = temporal_intents
    data.loc[batch.index, "temporal_intent"] = data.loc[
        batch.index, "temporal_intent"
    ].apply(json.dumps)
